    ...
    <View style={common.screen}>
      ...
      <View style={s.topHeader}>
        ...
        <TouchableOpacity style={s.quickLogHeaderBtn} onPress={() => setQuickLogModal(true)}>
          <Text style={{ color: '#fff', fontWeight: '800', fontSize: 12 }}>⚡ Quick Save Log</Text>
        </TouchableOpacity>
      </View>
      ...
      >
        ...
        <Text style={[s.titleHeader, { marginTop: 24, marginBottom: 12 }]}>Farm Overview</Text>
        <View style={s.statRow}>
          ...
          <View style={{ width: 12 }} />
          <View style={common.statCard}>
            <Text style={common.statLabel}>Eggs Collected</Text>
            <Text style={[common.statValue, { color: colors.amber }]}>{(summary?.totalEggs || 0).toLocaleString()}</Text>
            <Text style={common.statSub}>FCR: {summary?.feedConversionRatio || 0}</Text>
          </View>
        </View>
...
const s = StyleSheet.create({
  ...
  quickLogHeaderBtn: { backgroundColor: colors.brand, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 10 },
