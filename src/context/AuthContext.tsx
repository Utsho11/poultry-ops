...
import { IAuthUser as AuthUser, AnimalType, UserRole } from '../types';
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const restoreSession = async () => {
      try {
        ...
        if (savedToken && savedUser) {
          ...
          if (savedFarm) {
            try {
              const farmObj = JSON.parse(savedFarm);
              setActiveFarm(farmObj);
              setActiveFarmIdMemory(farmObj?._id || null);
            } catch {
              setActiveFarm(null);
              setActiveFarmIdMemory(null);
            }
          } else {
            setActiveFarm(null);
            setActiveFarmIdMemory(null);
  ...
  const switchFarm = async (farm: IFirm, newToken?: string) => {
    setActiveFarm(farm);
    setActiveFarmIdMemory(farm._id);
